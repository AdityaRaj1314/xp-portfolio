...
import MyMusic from "@/programs/MyMusic";
import MySkills from "@/programs/MySkills";
import TurnOffDialog from "components/TurnOffDialog/TurnOffDialog";
...
export default function Home() {
  const turnOffDialogOpen = useSelector((state: RootState) => state.system.turnOffDialogOpen);
  ...
  useEffect(() => {
    if (appState === "BOOT") {
      store.dispatch(setTurnOffDialogOpen(false));
    }
    if (appState === "DESKTOP") {
  ...
  if (appState === "LOGIN") {
    return <LoginScreen onLogin={() => setAppState("WELCOME")} onShutdown={() => setAppState("BOOT")} />;
  }

  return (
    <>
      <Head>
        <title>Aditya Raj&apos;s Personal Website - Home Page</title>
        <meta name="description" content="My Personal Space" />
        <meta name="viewport" content="width=device-width, initial-scale=1" />
        <link rel="icon" href="/images/favicon.ico" />
      </Head>
      {appState === "WELCOME" && <WelcomeScreen onComplete={() => setAppState("DESKTOP")} />}
            ...
            return tab.isMinimized ? (
              ...
              >
                {tab.program === App.MYWORK ? (
                  ...
                  <Paint />
                ) : tab.program === App.MYMUSIC ? (
                  <MyMusic />
                ) : tab.program === App.MYSKILLS ? (
                  <MySkills id={tab.id} />
